...
if ($#ARGV != 1 ) {
	print "Usage perl struct_assign.pl genelist.txt <path for SCRATCH1D run_SCRATCH-1D_predictors.sh>\n";
	exit;
}
##############################################################################


my($gene_list,$scratch_path) = @ARGV;
...
{
my $file = $locus . ".nex"; # Input nexus file
my $file_fasta = "$locus.fasta"; # Consensus sequence created for input file using Henikoff and Henikoff weights
my $updated_nex = $locus . ".updated" . ".txt";
my $output_file = $locus . ".out" . ".nex"; # Output nexus file with structural classes CHARSETS
my $SS_file = $file_fasta . ".out" . ".ss"; # Output file from scratch SSPro
my $ACC_file = $file_fasta . ".out" . ".acc"; # Out file from Scratch ACCPro
...
############ Create a copy of original nexus file and adding assumptions block#########
print"Creating copy of original nexus file as $output_file\n";
system("cp $dir/$file $dir/$output_file");

open(my $FH,">>","$dir/$output_file") or die "Couldn't open $!\n";
{
print $FH "\nBEGIN ASSUMPTIONS;\n";
...
open(my $FH2,">>","$dir/$output_file") or die "Couldn't open $!\n";
{
...
system("run_SCRATCH-1D_predictors.sh $input $out 4");
